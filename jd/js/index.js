...
(function() {
	var reg = /[,]/g;
	var co = getCookie('cartsid').replace(reg, '');
	$('.ci-count').html(co.length);
})();

(function () {
	 $('.jdm-toolbar-tab').hover(function  () {
	 	$('.jdhy').eq($(this).index()).show();
	 		$('.jdhy').eq($(this).index()).stop().animate({left:-50},200)
	 },function  () {
	 	$('.jdhy').eq($(this).index()).stop().animate({left:0},200);
	 	$('.jdhy').eq($(this).index()).hide();
